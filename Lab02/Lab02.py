...
def cross_entropy(w, X, y):
    ...
    eps = 0.000001  # Small constant to avoid log(0)
    # Because this involves taking logs, you should add 'eps' where necessary to
    # avoid taking the log of 0.
    cross_entropy_loss = -np.mean(y * np.log(p + eps) + (1 - y) * np.log(1 - p + eps))
...
def gradient(w, X, y):
    ...
    grad = np.dot(X.T, (p - y)) / n   # Averaging over all observations
    return grad 
...
def predict_class(w, X, t):
    ...
    return (probabilities >= t).astype(int)
...
def question_6():
    return 8.67864029967296 # <- change this with estimated bias for logit of class=0
...
def question_7():
    return {
      ...
      'Acc': 0.95556     # replace with the accuracy for that pair
      }    
